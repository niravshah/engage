...
;( function( window ) {
	...
	function TiltSlider( el, options ) {
		...
		// the items
		this.items = this.el.querySelector( 'ol.slides00' ).children;
		// total items
	...
	TiltSlider.prototype._addNavigation = function() {
		...
		for( var i = 0; i < this.itemsCount; ++i ) {
			inner += i === 0 ? '<span class="current11"></span>' : '<span></span>';
		}
	...
	TiltSlider.prototype._showItem = function( pos ) {
		...
		classie.removeClass( this.navDots[ this.current ], 'current11' );
		...
		classie.addClass( this.navDots[ this.current ], 'current11' );
		...
		var cntAnims = 0,
			...
			onEndAnimationCurrentItem = function() {
				++animEndCurrentCnt;
				var endFn = function() {
					classie.removeClass( currentItem, 'hide22' );
					classie.removeClass( currentItem, 'current11' );
					isFinished();
				};
			...
			onEndAnimationNextItem = function() {
				++animEndNextCnt;
				var endFn = function() {
					classie.removeClass( nextItem, 'show22' );
					classie.addClass( nextItem, 'current11' );
					isFinished();
				};
		...
		classie.addClass( currentItem, 'hide22' );
		classie.addClass( nextItem, 'show22' );
